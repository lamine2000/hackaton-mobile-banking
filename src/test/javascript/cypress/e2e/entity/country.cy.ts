...
describe('Country e2e test', () => {
  ...
  const countrySample = { name: 'Bacon encoding', codeAlpha: 'Handmade', code: 'COM vortals Oman' };
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      ...
      cy.get(`[data-cy="codeAlpha"]`).type('Cuba transparent system').should('have.value', 'Cuba transparent system');

      cy.get(`[data-cy="code"]`).type('Népal sticky a').should('have.value', 'Népal sticky a');

      cy.get(`[data-cy="flag"]`).type('online').should('have.value', 'online');

      cy.get(`[data-cy="createdAt"]`).type('2023-01-16T17:13').blur().should('have.value', '2023-01-16T17:13');

      cy.get(`[data-cy="createdBy"]`).type('Electronics programming').should('have.value', 'Electronics programming');
