...
describe('Department e2e test', () => {
  ...
  const departmentSample = { name: 'synthesize', code: 'state engine' };
  ...
  describe('new Department page', () => {
    ...
    it('should create an instance of Department', () => {
      cy.get(`[data-cy="name"]`).type('web-enabled e-commerce synergize').should('have.value', 'web-enabled e-commerce synergize');

      cy.get(`[data-cy="code"]`).type('payment').should('have.value', 'payment');

      cy.get(`[data-cy="createdAt"]`).type('2023-01-16T11:44').blur().should('have.value', '2023-01-16T11:44');

      cy.get(`[data-cy="createdBy"]`).type('b protocol').should('have.value', 'b protocol');
